...
public sealed class CreateCashDepositHandler : IRequestHandler<CreateCashDepositCommand, Result<CashSessionResponse>>
{
    ...
    public CreateCashDepositHandler(
    ...
    public async Task<Result<CashSessionResponse>> Handle(CreateCashDepositCommand request, CancellationToken cancellationToken)
    {
        ...
        if (session is null)
            return Result<CashSessionResponse>.Failure(CreateCashDepositErrors.SessionNotFound);
        ...
        try
        {
            session.RegisterDeposit(request.Amount, request.Description, _currentUserService.UserId);
        }
        catch (Exception ex) when (ex is ArgumentException or InvalidOperationException)
        {
            return Result<CashSessionResponse>.Failure(
                Error.Conflict(CreateCashDepositErrors.InvalidOperation.Code, ex.Message));
        }
