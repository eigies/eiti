...
using eiti.Application.Features.CashSessions.Commands.CreateCashDeposit;
...
public sealed class CashSessionsController : ControllerBase
{
    ...
    public async Task<IActionResult> Deposit(Guid id, [FromBody] CreateCashDepositCommand command, CancellationToken cancellationToken)
    {
        var result = await _sender.Send(command with { Id = id }, cancellationToken);
        return result.ToActionResult();
    }
